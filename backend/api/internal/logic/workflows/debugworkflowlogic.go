...
func (l *DebugWorkflowLogic) DebugWorkflow(workflowID uint64, req *types.ExecuteReq) (resp *types.ExecuteResp, err error) {
	userID, err := getUserIDFromCtx(l.ctx)
	if err != nil {
		return nil, err
	}
	out, err := l.svcCtx.WorkflowRpc.DebugWorkflow(l.ctx, &workflowrpc.ExecuteReq{
		UserId:     userID,
